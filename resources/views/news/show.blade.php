...
@section('content')
    ...
    <hr>
    <p>Avtor: <a><strong>{{$data['news'][0]['user']['username']}}</strong></a>, {{strftime('%d/%m/%G ob %H:%M', strtotime($data['news'][0]['created_at']))}}</p>
    @if ($data['news'][0]['updated_at'] != null)
    <p>Zadnja sprememba: {{strftime('%d/%m/%G ob %H:%M', strtotime($data['news'][0]['updated_at']))}}</p>
    @endif
    <hr>
    <h5>{{$data['news'][0]['summary']}}</h5>
    @if ($data['news'][0]['img_ref'] != 'default.png')
